...
export const BookSection: React.FC<BookSectionProps> = ({ category, books }) => {
  const { setSelectedCategory } = useApp();

  if (books.length === 0) return null;

  return (
    <div className="bg-[#FFFDF8] rounded-3xl border border-jain-border/90 p-3.5 sm:p-5 shadow-jain-card mb-6">
      {/* Section Header Row matching Screenshot 1 */}
      <div className="flex items-center justify-between pb-3.5 mb-3.5 border-b border-jain-border/60">
        <div className="flex items-center gap-2.5 sm:gap-3">
          <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-jain-cream border border-jain-gold/40 flex items-center justify-center p-1.5 shadow-xs">
        ...
        <button
          ...
          className="bg-[#FFF8EC] hover:bg-[#FFF0B8] text-jain-maroon active:scale-95 text-xs font-bold px-3 py-1.5 rounded-xl border border-jain-gold/30 transition-all flex items-center gap-1 shadow-xs"
        >
          <span>विस्तार</span>
